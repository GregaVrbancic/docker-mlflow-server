...
CMD /bin/bash ./wait-for-it.sh ${DB_HOST}:${DB_PORT} -- mlflow db upgrade postgresql+psycopg2://${DB_USER}:${DB_PASSWD}@${DB_HOST}/${DB_NAME} || true && \
    mlflow server --backend-store-uri postgresql+psycopg2://${DB_USER}:${DB_PASSWD}@${DB_HOST}/${DB_NAME} --default-artifact-root ${ARTIFACT_ROOT} --host 0.0.0.0
